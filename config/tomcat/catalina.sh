...
#JAVA_OPTS="$JAVA_OPTS -Dfcrepo.modeshape.index.directory=/usr/local/fcrepo4-data-plano/fcrepo.index.directory"
...
# pc: set java options for openrdf-sesame
# JAVA_OPTS="$JAVA_OPTS -Dinfo.aduna.platform.appdata.basedir=/usr/local"

# pc: set fcrepo username and password for indexer
# JAVA_OPTS="$JAVA_OPTS -Dfcrepo.username=fedoraAdmin -Dfcrepo.password=fedoraAdmin"

# pc: rdf/jcr/file indexer storage
# JAVA_OPTS="$JAVA_OPTS -Dfcrepo.rdf.storage=/usr/local/fcrepo-rdf"
# JAVA_OPTS="$JAVA_OPTS -Dfcrepo.jcrxml.storage=/usr/local/fcrepo-jcrxml"
# JAVA_OPTS="$JAVA_OPTS -Dfile.serializer.dir=/usr/local/fcrepo-file"

# pc: set variable and java options for marmotta
# export JAVA_OPTS="$JAVA_OPTS -Djava.net.preferIPv4Stack=true -XX:+UseConcMarkSweepGC -XX:+CMSClassUnloadingEnabled -XX:+HeapDumpOnOutOfMemoryError"
# export MARMOTTA_HOME=/usr/local/marmotta

# pc: set variables for remote jmx

CATALINA_PID=/usr/local/tomcat7/bin/pid
CATALINA_OPTS="-Dcom.sun.management.jmxremote -Dcom.sun.management.jmxremote.ssl=false -Dcom.sun.management.jmxremote.authenticate=true -Dcom.sun.management.jmxremote.password.file=/usr/local/tomcat7/conf/jmxremote.password -Dcom.sun.management.jmxremote.access.file=/usr/local/tomcat7/conf/jmxremote.access"

# pc: set JPDA remote debugger (jpda start)
export JPDA_ADDRESS=1044

# pc: remove log files before start
if [ "$1" = "start" ] || [ "$1" = "run" ] || [ "$2" = "start" ]; then
  rm -f /usr/local/tomcat7/logs/*
fi
